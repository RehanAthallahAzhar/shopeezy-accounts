...
func (s *jwtTokenService) GenerateToken(ctx context.Context, user *models.User) (string, error) {
	claims := &JWTClaims{
		UserID:   user.ID,
...
func (s *jwtTokenService) ValidateToken(ctx context.Context, tokenString string) (isValid bool, userID string, username string, role string, errorMessage string, err error) {
	...
	if err != nil {
		...
		return false, "", "", "", "Token invalid or expired", err
	}

	claims, ok := token.Claims.(*JWTClaims)
	if !ok || !token.Valid {
		log.Printf("Invalid token or claims mismatch: %v", err)
		return false, "", "", "", "Invalid token", nil // No Go error, just invalid token
	}
	...
	if jti != "" {   // JTI might be empty if not set during token creation
		...
		if err != nil {
			...
			return false, "", "", "", "Internal server error during token validation", err
		}
		if isBlacklisted {
			log.Printf("Token with JTI %s is blacklisted.", jti)
			return false, "", "", "", "Token has been revoked", nil // No Go error, just invalid token
		}
	}
